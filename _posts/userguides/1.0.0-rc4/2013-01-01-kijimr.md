 ...
 * `Map<String, KeyValueStore<?, ?>> getRequiredStores()`. This method should construct and return a map specifying all the `KeyValueStore`s that this producer wants to access. `KeyValueStore`s are a complex subject and have their own section later in this guide.
